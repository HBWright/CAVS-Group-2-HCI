...
function OperatorMap() {
    ...
    ros.on('connection', () => {
      ...
      TOPICS.forEach(({ name, rosTopic, messageType }) => {
        ...
        topic.subscribe((msg: any) => {
          ...
          // Camera display in OperatorBoxCam
          ...
          if (name === 'GPS' && msg.latitude && msg.longitude) {
            const gpsPoint = { lat: msg.latitude, lon: msg.longitude, alt: msg.altitude };
            setGpsData(gpsPoint);
          ...
          // IMU data for speed calculation (simplified)
          if (name === 'Imu' && msg.linear_acceleration) {
            // Approximate speed from acceleration (very simplified)
            const accel = Math.sqrt(
              Math.pow(msg.linear_acceleration.x, 2) +
              Math.pow(msg.linear_acceleration.y, 2)
            );
            // This is a rough approximation - ideally integrate acceleration over time
            setSpeed(Math.abs(accel * 2.237)); // Convert m/s² to mph (very rough)
          }

          // Path planning visualization (using Lidar for now)
  ...
  return (
    ...
    <div className={styles.ProfilePageContainer}>
      ...
      <div className={shapes.OperatorDiagText}>
  ...
  <span style={{ color: topicStatus.GPS ? '#00ff00' : '#ff0000' }}>GPS</span>
      ...
      {/* GPS Map Display */}
      <div className={shapes.OperatorBoxGPS}>
        <canvas 
          ref={gpsCanvasRef}
          width={1200}
          height={800}
      ...
      {/* Camera Display */}
      <div className={shapes.OperatorBoxCam}>
        <canvas 
          ref={cameraCanvasRef}
          width={640}
          height={480}
      ...
      {/* Local Path Display */}
      <div className={shapes.OperatorBoxPath}>
        <canvas 
          ref={pathCanvasRef}
          width={600}
          height={600}
